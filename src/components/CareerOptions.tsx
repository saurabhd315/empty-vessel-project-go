...
export const CareerOptions = () => {
  ...
  const filteredCareers = careers.filter(career => {
    const matchesSearch = career.title.toLowerCase().includes(searchQuery.toLowerCase()) || 
                         career.teaser.toLowerCase().includes(searchQuery.toLowerCase());
          ...
          <div className="filter-container" ref={filterRef}>
            ...
            {isFilterOpen && (
              <div className="filter-dropdown">
                <div 
                  className={`filter-option ${!industryFilter ? 'selected' : ''}`}
                  onClick={() => {
                    setIndustryFilter("");
                    setIsFilterOpen(false);
                  }}
                ...
                {industries.map(industry => (
                  <div 
                    key={industry} 
                    className={`filter-option ${industryFilter === industry ? 'selected' : ''}`}
                    onClick={() => {
        ...
        <div className="career-grid">
          <AnimatePresence>
            {filteredCareers.map((career, index) => (
              ...
              >
                <Link to={`/careers/${career.id}`} className="career-card" style={{ backgroundColor: career.color }}>
                  <div className="career-icon" style={{ color: "#0778b6" }}>
                    {career.icon}
                  </div>
