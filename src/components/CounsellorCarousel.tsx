...
export const CounsellorCarousel = () => {
  ...
  const renderRatingStars = (rating: number) => {
    return Array(5).fill(0).map((_, i) => <Star key={i} className={`w-4 h-4 ${i < rating ? "text-yellow-400 fill-yellow-400" : "text-gray-300"}`} />);
  };
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="carousel-container">
          ...
          loop: true
            ...
            <CarouselContent>
              {counsellors.map((counsellor: Counsellor) => <CarouselItem key={counsellor.id} className={`${isDesktop ? 'basis-1/3' : isTablet ? 'basis-1/2' : 'basis-full'} p-2`}>
